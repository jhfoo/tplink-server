<template>
  <div>
    ...
    <v-data-table :headers="TableLabels" :items="devices" item-key="host" dense>
    ...
    <v-bottom-sheet v-model="isShowSheet" inset>
      <v-sheet>
        <v-card>
          <v-card-title>Device Properties</v-card-title>
          <v-simple-table fixed-header height="400px" dense>
            ...
            <div class="text-center">
              <v-btn color="primary" rounded @click="isShowSheet = false">close</v-btn>
            </div>
            <v-card-actions>
            </v-card-actions>
          </v-card>
    ...
    methods: {
      ...
      onToggleOnOff(item) {
        if ('RelayState' in item) {
          ...
          .then((resp) => {
            if (resp.data === true) {
              // update device by removing from hash and adding back to trigger refresh
              // let device = JSON.parse(JSON.stringify(this.devices[item.host]))
              // device.RelayState = NewState
              // delete this.devices[item.host]
              item.RelayState = NewState
              this.$forceUpdate()
            }
          })
          .catch((err) => {
      ...
      updateDevices(isForce) {
        ...
        axios.get(`${API_BASEURL}/api/device/list`)
        .then((resp) => {
